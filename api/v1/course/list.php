<?php
    ...
    $listCountSQL = $con->prepare("SELECT count(id) FROM course $WHERE_CLAUSE $queryCondition");
    ...
    if($listCountSQL->num_rows() >= 1 && $listCountSQL->errno == 0){
        ...
        $listSQL = $con->prepare("SELECT name, course_code, department, id FROM course $WHERE_CLAUSE $queryCondition $limit");
        $listSQL->execute();
        $listSQL->bind_result( $name, $course_code, $department, $id);
