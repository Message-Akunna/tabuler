<?php
    ...
    //Description: List lecture halls table
    ...
    $listCountSQL = $con->prepare("SELECT count(id) FROM lecture_halls $WHERE_CLAUSE $queryCondition");
    ...
    if($listCountSQL->num_rows() >= 1 && $listCountSQL->errno == 0){
        ...
        $listSQL = $con->prepare("SELECT hall_name, capacity, id FROM lecture_halls $WHERE_CLAUSE $queryCondition $limit");
        $listSQL->execute();
        $listSQL->bind_result( $hall_name, $capacity, $id);
