<?php
    ...
    $listCountSQL = $con->prepare("SELECT count(id) FROM period $WHERE_CLAUSE $queryCondition");
    ...
    if($listCountSQL->num_rows() >= 1 && $listCountSQL->errno == 0){
        ...
        $listSQL = $con->prepare("SELECT period_name, period_time, id FROM period $WHERE_CLAUSE $queryCondition $limit");
        $listSQL->execute();
        $listSQL->bind_result($period_name, $period_time, $id);
        ...
        if($listSQL->num_rows() >= 1 && $listSQL->errno == 0)
        {
            while($listSQL->fetch()) 
            {
                ...
                $result['period_time'] = (string)$period_time; 
