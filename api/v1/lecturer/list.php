<?php
    ...
    //Description: List lecturer table
    ...
    $listCountSQL = $con->prepare("SELECT count(id) FROM lecturer $WHERE_CLAUSE $queryCondition");
    ...
    if($listCountSQL->num_rows() >= 1 && $listCountSQL->errno == 0){
        ...
        $listSQL = $con->prepare("SELECT department, course, reg_no, id FROM lecturer $WHERE_CLAUSE $queryCondition $limit");
        $listSQL->execute();
        $listSQL->bind_result( $department, $course, $reg_no, $id);
